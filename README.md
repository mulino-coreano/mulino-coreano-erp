...
| 레이어 | 구성 | 역할 |
|---|---|---|
| L0 | PostgreSQL 18(ERP 30 + 인터페이스 13 + 계획 9 + 멱등 1) + Spring Boot + MCP Server | ERP 데이터 및 기능을 Tool로 노출 |
...
- **DB**: PostgreSQL 18 (ERP 30 + 인터페이스 13 + 계획 9 + 멱등 1개 테이블)
...
현재 구현은 PoC·로컬 전용 신원 모델을 사용한다. 인간 요청은 `X-Mulino-Local-Role` 헤더로 역할을 전달하고(`local-{role}@mulino.local`로 upsert), 실행기는 `Authorization: Bearer $MULINO_WORKER_TOKEN`(constant-time compare)으로 인증한다. 역할 헤더는 누구나 전송할 수 있으므로 실제 API 공개에는 외부 신원 제공자가 필요하다(#21·#22 보류). 원격 MCP HTTP transport와 Auth0 OBO는 #22 범위로 보류됐다.

Backend는 DB 설정과 `MULINO_WORKER_TOKEN`이 필요하며, stdio MCP는 `MULINO_LOCAL_ROLE` 환경 변수를 요구한다. 인증을 생략하는 개발용 변경 API는 제공하지 않는다. 세부 구현 순서는 [재보충 데모 계획](docs/superpowers/plans/2026-09-05-replenishment-demo.md)을 따른다.

주문 이력·다단계 BOM·공유 재고·복수 공급처를 연결하는 서버 계산과 재현 가능한 fixture를 구현했습니다. [계산 구현 안내](docs/12_replenishment_calculation.md)에 산식·데이터 대사·기대 결과를 정리했습니다. 불변 계획 저장과 scoped Run API·Node 실행기에 이어 [최소 Zig CLI와 Codex 이미지](docs/14_cli_and_runtime.md)도 연결했습니다. [실행 연결 안내](docs/13_execution_and_plan_api.md)에 계약과 검증 범위를 정리했습니다. 구매 제안·결정·발주 반영 REST 경로를 연결하고 구매 데이터 접근은 생성된 jOOQ 타입으로 정리했습니다. clean 빌드의 타입 생성에는 Docker가 필요합니다. 실제 로그인·모델 업무 수행과 대화 승인 도구 연결은 후속입니다.
