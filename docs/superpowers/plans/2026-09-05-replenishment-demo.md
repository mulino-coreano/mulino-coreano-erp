...
> 상태: 2·3단계 구현 완료, 4단계 로컬 서버·Node 실행기와 최소 Zig CLI·Codex 이미지 구현. 구매 제안·MANAGER 결정·발주 반영과 데이터 접근 정리를 완료했다. 구매 CLI와 인간 MCP 도구를 연결했다. 서버 관리 후속 의무까지 연결했다. 실제 Auth0/두 클라이언트 로그인·모델 실행과 전체 인수는 미완료 · 2026-09-06
...
- [x] Orchestrator → Supply Chain → Procurement의 역할별 업무·hand-off·승인 대기를 연결한다. 대기 중 역할 실행의 종료와 상위 Case 책임의 지속을 스킬에 구분한다. 역할 변경을 업무 흐름에도 반영한다.
...
- [x] 불변 발주 묶음 제안, governance action, AUTHORITY_REQUIRED Attention, APPROVAL 대기를 한 트랜잭션으로 만든다. 같은 plan version의 제안 중복을 방지한다.
- [x] 승인 대상은 공급처별 PO 전체를 포함한 정확한 plan 구매 묶음이다. 잠금 순서는 요청 조정 → source guard → 창고 조정 → WI/Run/Case/Agent → plan → governance action → 관련 행을 따른다. 조건 재검증 후 PO·상세·decision·audit·purchase_application·Event를 원자적으로 저장한다.
- [x] `created_by`는 승인 인간으로 기록하고 요청자·제안 agent는 연결 이력에 남긴다. 전자세금계산서 번호·일자는 실제 발행 전 NULL로 두고 가짜 값을 생성하지 않는다.
- [x] 반려·만료·변경안 재승인, 승인과 반려 경쟁, 응답 유실 후 재호출, 중간 INSERT 실패, 다른 사람이 전달한 actor ID, worker/OPERATOR의 승인 접근을 테스트한다.
- [x] 원래 payload의 hash만 비교하지 않고 현재 DB 입력도 다시 검증한다. 구매 검증 서비스만 해당 업무를 DONE으로 전환할 수 있게 한다.
...
- [x] 발주안에는 필요한 인간 권한, 추천 내용, 계산 근거, 미조치 영향과 남은 불확실성을 표시한다. 설명에서 재고 현재값과 예정 공급을 구분한다.
- [ ] ChatGPT에서 접수 → Codex의 새 대화에서 찾기·검토·승인 → ChatGPT에서 결과 확인을 시연한다. 반대 방향도 시험한다. 결정 도구의 확인을 기억하거나 자동 승인하도록 설정하지 않는다.
- [x] 일반 Attention 답변은 THIS_ACTION/THIS_CASE 범위만 지원하고, 승인 도구 우회가 불가능한지 검사한다. 발주 완료 후 남은 생산·입고 책임과 기한을 보여준다.

**산출 계약:** 사용자 관점에서 목표·근거·결정·현재 상태가 대화 경계를 넘어 이어진다.

### 8단계 — 전체 검증과 인수 문서

아래 체크는 로컬 검증 및 문서 산출의 완료만 뜻한다. 백엔드 532개·로컬 E2E 2개의 최신 근거는 [실행 API 안내](../../13_execution_and_plan_api.md), 시험과 실제 계정의 경계는 [실행서](../../15_demo_runbook.md)에 기록했다. 실제 모델·두 클라이언트 인수가 남아 있으므로 8단계 전체는 미완료다.
...
- [x] 요구사항의 구매 예시를 원재료 기준으로 수정하고 구현 현황·흐름도·ERD·디스패처·MCP/CLI 문서를 갱신한다. 기능/UX별 실제 시험 근거를 연결한다.
- [x] 각 검증 단위별 PR은 저장소의 한국어 PR 템플릿 4개 섹션과 3개 체크 항목을 사용한다. 계획 밖 품질·생산·출고 기능을 함께 구현하지 않는다.
...
- 서버 관리 후속 책임은 이후 V25에 연결했다. 실제 모델·클라이언트 전체 시연이 남아 있어 7·8단계 전체를 완료로 표시하지 않는다. 최신 검사 결과는 실행·계획 API 안내를 따른다.


### 발주 이후 후속 책임 — V25

[후속 책임 구현 계약](2026-09-06-replenishment-followup.md)에 따라 검증된 구매 완료와 후속 저장을 같은 트랜잭션으로 연결했다. Case WAITING, 서버 소유 Orchestrator 조정 업무, 실제 납기·입고/LOT 관찰과 중복 없는 주의 요청을 검증했다. 구매 불필요와 생산 불필요를 구분하고 Case를 자동 종결하지 않는다. 전체 모델·클라이언트 시연은 이 로컬 검사와 별도다.


### 전체 로컬 경로 검증과 구조 정리 — 2026-09-06

- 실제 PostgreSQL 18·Spring HTTP·Streamable HTTP MCP·Node 실행기·Zig CLI를 연결한 별도 E2E 2개를 통과했다. 인증 발급과 모델 판단만 시험용이다. 승인 전/반려 후 PO 0건, 정확한 발주 묶음, 중복 요청, 가격 변경 후 새 승인, 승인 대기 중 실제 Spring 재시작 후 복구를 확인했다.
- 전체 백엔드 519개와 E2E 2개가 실패·오류·skip 없이 통과했다. MCP 26개, 실행기 48개, CLI smoke 32개, Auth0 설정 19개와 준비 검사 20개도 통과했다. 이미지 권한·환경·취소 smoke는 모델 요청 없이 통과했다.
- 실제 JWT 권한 변환에서 MANAGER의 `procurement:decide`를 검증하고, 만료·반려 방침 질문을 미완료 Orchestrator 부모 업무에 연결해 늦은 인간 답변 후 재계산이 가능하도록 수정했다.
- 실행 서비스의 모델 대기 조건 검증을 `RunWaitingPolicy`로 분리했다. 기존 호출 트랜잭션, 최종 lease 확인, 상태 변경과 이벤트 순서는 유지하고 전체 백엔드·E2E를 다시 통과했다. 이는 구조 정리의 첫 변경이며 전체 구조 정리 완료를 뜻하지 않는다.
- [데모 실행 안내](../../15_demo_runbook.md)는 읽기 전용 준비 검사와 실제 계정 인수를 구분한다. 실제 Auth0 테넌트·고정 MCP 주소·전용 Codex 로그인 설정, 두 클라이언트의 명시적 승인 UX와 실제 모델/native 역할 실행 증거가 아직 필요하다. 이 항목들은 로컬 E2E로 완료 처리하지 않는다.

### 검토와 인수 현황 — 2026-09-12

3명 독립 검토와 후속 범위 확인으로 디스패치 입력·종료 Case·담당자 조회·일반 답변 승인 해석·상세 이력의 문제를 수정했다. 승인 요청의 원래 계획 근거·미조치 영향 표시도 보완했다. 변경과 최신 검증은 [PR 검토 및 인수 기록](../../reviews/2026-09-12-pr18-pr19.md)에 연결했다.

| 단계 | 로컬 구현/검증 | 남은 인수 |
|---|---|---|
| 1 인증 | JWT/ERP 역할·OBO/설정 도구 로컬 시험 완료 | 실제 tenant·클라이언트 로그인/갱신·신원 보존 |
| 2 계획 데이터 | Flyway/독립 DDL 정의 비교·fixture 검증 완료 | 없음(정해진 데모 데이터 범위) |
| 3 계획 계산/저장 | 계산·불변 버전·출처·권한/실패 검증 완료 | 없음(정해진 계산 범위) |
| 4 실행 서버 | lease·재시도·종료/대기·Node 실행기 검증 완료 | 실제 모델 역할 수행과 native 분석 분담 |
| 5 CLI/이미지 | CLI와 0.154.0 이미지·전용 로그인 검증 완료 | 크레딧이 있는 워크스페이스에서 실제 Astra 수행 |
| 6 구매 승인 | 승인·반려·변경 만료·재승인·원자적 발주/감사 로컬 검증 완료 | 인간 클라이언트의 명시적 결정 UX는 7단계 |
| 7 인간 대화 | 조회·승인 근거·일반 답변·후속 책임 표시 로컬 구현 | 두 클라이언트 왕복·개별 확인/취소 |
| 8 전체 인수 | 로컬 E2E·문서·준비 검사 완료 | 앞 단계 실제 계정/모델/클라이언트 인수 증거 |

실제 Codex 로그인은 성공했으나 Astra 요청은 워크스페이스 크레딧 부족으로 종료됐다. Auth0는 로그인 전 상태다. ngrok는 후속 Safari 로그인·기존 고정 주소 설정과 실제 HTTPS 전달 검증을 완료했다. 실제 계정 설정과 인간 결정은 fixture로 대체하지 않으며 전체 단계 완료도 표시하지 않는다.
