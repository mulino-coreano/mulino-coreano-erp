...
계산 결과를 불변 계획 버전으로 저장하고, Case·Work Item에 묶인 실행 권한으로 업무를 처리하는 서버 경로와 Node 실행기를 구현했다. 실제 Codex용 CLI·컨테이너 이미지 및 Auth0/두 대화 클라이언트의 실연결은 아직 완료하지 않았다. 구매 승인·발주 적용은 후속 단계다.
...
현재 자동 시험은 실제 Node 자식 프로세스와 모의 Auth0/백엔드를 사용한다. 실제 Docker 이미지가 없으므로 이를 실제 Codex 실행 검증으로 표시하지 않는다. CLI 구현·역할 프롬프트·이미지 빌드 및 실제 claim 문맥 크기 확인은 다음 단계다.

## 검증과 마이그레이션

- PostgreSQL 18의 전체 Backend `clean test bootJar`: 405개 통과, 실패·오류·skip 0. 역할별 후속 업무 생성 시 Case 참여자와 예약 맥락이 함께 저장되는 회귀도 포함한다.
- Node 실행기 41개, MCP 19개 테스트 통과.
