...
계산 결과를 불변 계획 버전으로 저장하고, Case·Work Item에 묶인 실행 권한으로 업무를 처리하는 서버 경로와 Node 실행기를 구현했다. 구매 제안·MANAGER 결정·발주 적용 REST 경로도 연결했다. 최소 Zig CLI와 Codex 컨테이너 이미지는 [CLI·런타임 안내](14_cli_and_runtime.md)를 따른다. 2026-09-25 Claude 런타임으로 재보충 플로우 실제 인수를 수행했다. 발주 승인은 인간에게 보류했다. Codex 실제 업무 수행 인수는 크레딧 소진으로 미완료다. 실제 Auth0·대화 클라이언트 로그인 인수는 보류(#21·#22)다.
...
- 독립 DDL 00~17·seed와 Flyway 경로를 별도 PostgreSQL 18 DB에서 검증했다. 2026-09-25 Claude 런타임으로 재보충 플로우 실제 인수를 수행했다. 발주 승인은 인간에게 보류했다. Codex 실제 업무 수행 인수와 전체 시연은 아직 남아 있다. 외부 신원 제공자(Auth0 등)·ChatGPT·Codex 로그인 인수는 보류(#21·#22)다.
