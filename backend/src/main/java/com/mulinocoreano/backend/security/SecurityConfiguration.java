...
public class SecurityConfiguration {
    ...
    JwtDecoder jwtDecoder(AuthProperties properties) {
        return ErpJwtDecoder.create(
                properties.issuer(), properties.audience(), properties.jwksUri());
    }

    @Bean
    @Order(1)
    SecurityFilterChain agentSecurity(HttpSecurity http, DatabaseRunCapabilityAccess capabilities)
            throws Exception {
    ...
    SecurityFilterChain apiSecurity(
            HttpSecurity http, ActorJwtConverter actors, AuthProperties properties)
            throws Exception {
        ...
        return http.csrf(csrf -> csrf.disable())
                ...
                .oauth2ResourceServer(
                        resource ->
                                resource.jwt(jwt -> jwt.jwtAuthenticationConverter(actors))
                                                ...
                                                (request, response, exception) -> {
                                                    if (exception
                                                                    ...
                                                                    OAuth2AuthenticationException
                                                                            oauth
                                                            && ActorJwtConverter.ERP_ACCESS_DENIED
                                                                    .equals(
                                                                            oauth.getError()
                                                                                    .getErrorCode())) {
                                                        response.setStatus(
