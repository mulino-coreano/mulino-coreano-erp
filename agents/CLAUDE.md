...
Claude Code and Codex sessions (Cowork) are both supported agent runtimes. This directory contains **no agent framework, no LLM client, no A2A, and no MCP implementation** — only the two things the agent runtime needs to act as ERP agents. An MCP Server is deferred pending a complexity-management decision.
...
- A skill teaches a Claude Code or Codex session its role; every backend interaction the session performs goes through `mulino`, which calls the Spring Boot REST API (L0).
...
- Agent-to-agent collaboration is the runtime's native subagent dispatch (Claude Code or Codex): the orchestrator is the main session and launches role subagents. There is no wire protocol between agents.
