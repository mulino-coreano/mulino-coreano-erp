...
Codex sessions (Codex / Cowork) are the agent runtime. This directory contains **no agent framework, no LLM client, no A2A, and no MCP implementation** — only the two things Codex needs to act as ERP agents. An MCP Server is deferred pending a complexity-management decision.
...
- A skill teaches a Codex session its role; every backend interaction the session performs goes through `mulino`, which calls the Spring Boot REST API (L0).
...
- Agent-to-agent collaboration is Codex's native subagent dispatch: the orchestrator is the main session and launches role subagents. There is no wire protocol between agents.
