...
Role skills describe Claude Code and Codex sessions (Cowork); the first automated runtime targets Codex. The local lease server and Node runner are implemented, but a minimal Zig CLI and dedicated runtime image have now been built and tested without model calls. Actual Codex model runs and native role hand-off acceptance remain pending. There is no custom agent framework or A2A protocol.
...
- The host config `MULINO_AGENT_API_URL` is mapped to the container's CLI variable `MULINO_API_URL`; `MULINO_TOKEN` contains only the scoped capability. Actual Docker file/permission/CLI/cancellation checks pass; the dedicated Codex login and real model execution remain separate acceptance work. See `../docs/14_cli_and_runtime.md`.
