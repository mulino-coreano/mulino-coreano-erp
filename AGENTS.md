...
**Current status**: Phase 4 is in progress. The Spring Boot backend implements Case intake, inventory lookup, event dispatch, Run scheduling and Auth0 JWT/ERP-role access control; `mcp-server/` provides authenticated stdio and Streamable HTTP with Auth0 OBO token exchange. Planning services load reconciled ERP snapshots, calculate historical demand/BOM/supplier candidates, and persist immutable scoped plan versions. Case intake queues Runs; leased execution APIs and a Node runner coordinate scoped agents, with Docker/CLI/live model integration still pending. PostgreSQL has 30 ERP tables plus 13 interface, one identity, nine planning and one request-idempotency table. Real tenant/client login validation remains separate from local tests. `governance/`, `dashboard/` and the Zig CLI remain scaffolds. Actual Docker/Codex execution, Zig CLI and purchase approval/write adapters are future work; see `docs/08_interface_overview.md` §13, `docs/11_auth0_setup.md` and `docs/12_replenishment_calculation.md` and `docs/13_execution_and_plan_api.md`. All business documentation is written in Korean.

## Commands

Use Java 21 and PostgreSQL 18. For the backend, create an empty DB and configure `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `MULINO_AUTH_ISSUER` and `MULINO_API_AUDIENCE` locally; Flyway applies V1–V22, including Orchestrator/role bootstrap, external identities, planning data, leases/idempotency and latest planning-attempt state. Follow `docs/11_auth0_setup.md` for Auth0/MCP configuration. Use a separate disposable DB for integration tests. The planning fixture must be loaded into an empty disposable business database before adding demo login identities; see `database/seed/replenishment_demo_README.md`.
...
|---|---|---|
| L0 | `database/`, `backend/` | PostgreSQL 18 (30 ERP + 13 interface + 1 identity + 9 planning + 1 idempotency tables) + Spring Boot REST API (single entry point for CLI and dashboard) |
